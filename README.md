...
| **BinTrasher**(String dir, int lumaFormat,String thresholdScaling) **throws IOException** | `dir` = path to your Bitmap, `lumaFormat` = *`as above`*, `thresholdScaling` = how much above/below the **minimum-luma**(the **initial-threshold**) to use as the new threshold<br> so if it's `50%` then the **new-threshold**= **old**-**threshold** +((**old**-**threshold**-**lowestLuma**)\*(**50**/**100F**)<br>  or if it's `-25%` then **new-threshold** = **old**-**threshold** +((**old**-**threshold**-**lowestLuma**)\*(**-25**/**100F**) | `get888BinaryImage()`,`get565BinaryImage()`,`getBinaryMatrix()`<br>`getGreyStratifiedImage(~)`,`getColoredStratifiedImage(~)` |
